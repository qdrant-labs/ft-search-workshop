...
- **Models compared:** **BM25** (FastEmbed `Qdrant/bm25` sparse, the traditional product-search baseline) · **Generic dense** (`all-MiniLM-L6-v2` via FastEmbed) · fine-tuned **SPLADE** (`thierrydamiba/splade-ecommerce-esci`) · **Hybrid (D+SPLADE)** — generic dense + fine-tuned SPLADE via server-side RRF.
...
| 20 min | **CP2 — BM25, generic dense, and fine-tuned SPLADE (first numbers)** | Same queries against BM25, the `dense` named vector, and the `splade_finetuned` named vector. Side-by-side three-way view with ESCI grades on each row. Inspect one fine-tuned sparse vector — show its active terms (sparse is interpretable; dense is opaque). **First metric reveal:** nDCG@10, MRR@10, Recall@10, and Precision@10 across **BM25, generic dense, and fine-tuned SPLADE** on the **10-query demo set** — explicitly labeled as illustrative (n=10), not the headline claim. |
| 13 min | **CP3 — Hybrid fusion** | Use Qdrant's Query API to fuse generic dense + fine-tuned SPLADE with RRF in a single `query_points` call — two `Prefetch` blocks + `FusionQuery(fusion=Fusion.RRF)`. Re-run quality metrics **4-way** on the demo set (illustrative): BM25 / Generic dense / SPLADE / Hybrid (D+SPLADE). |
| 15 min | **Wrap, bad queries demo, Q&A** | Final metrics table — **BM25 / Generic dense / SPLADE / Hybrid (D+SPLADE)** · nDCG@10 / MRR@10 / Recall@10 / Precision@10 with 95% CIs from the full 2K eval. Then show the query-level findings table: where SPLADE helped most, and where hybrid helped or hurt. No latency or cost columns; the workshop stays focused on quality (latency is a Q&A topic, not a column). **Bad queries demo (~3 min):** five bad queries against fine-tuned SPLADE return product results regardless of intent — corpus-mismatch story, not a "model is broken" story. Mention query routing + cross-encoder rerank as the "next layer" in Q&A. Stretch "your turn" if time. Preview the "fine-tune your own" path with the takeaway notebook. |
