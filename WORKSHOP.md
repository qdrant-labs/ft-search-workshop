...
- **Narrative arc:** BM25 baseline → generic dense + fine-tuned SPLADE → hybrid fusion.
- **Headline takeaway:** quantified lift on real metrics. Attendees see fine-tuned SPLADE beat BM25 and generic dense on graded ranking metrics over Amazon ESCI and believe the numbers.
- **Lab style:** three checkpoints with mini-exercises, structured but interactive.
- **Audience handling:** mixed-technical room (search engineers, ML engineers, data scientists, some product managers) following a **single path through the notebook** — no two-track engineer-leads / PM-watches split. Metric definitions are kept short and concrete; the wrap includes a query-level findings table so attendees can connect aggregate scores to real examples. Concept depth in the intro stays at one-liners; deeper concepts deferred to a future workshop.
- **Dataset:** Amazon ESCI (US test subset). The 2K test queries are loaded live from HuggingFace at notebook startup. The indexed corpus is roughly 35K-40K products — every product that appears in any test-query qrels row — so every test query's graded products are reachable. The full Amazon catalog is not indexed; the workshop tells attendees this up front so "your turn" queries about out-of-corpus products are not mistaken for model failures. Graded relevance labels (E/S/C/I) are used both in nDCG@10 *and* surfaced visually in result rows.
- **Models compared:** **BM25** (FastEmbed `Qdrant/bm25` sparse, the traditional product-search baseline) · **Generic dense** (`all-MiniLM-L6-v2` via FastEmbed) · fine-tuned **SPLADE** (`thierrydamiba/splade-ecommerce-esci`) · **Hybrid DBSF** — generic dense + fine-tuned SPLADE via server-side DBSF.
...
- **Hybrid fusion:** DBSF as the default. It normalizes scores from generic dense and SPLADE before combining them, and it measured better than RRF on the workshop corpus. RRF and weighted fusion are Q&A topics, not built into the lab.
- **Notebook environment:** built-in IDE on each cloned attendee machine, using the workshop Python environment.
- **Eval rigor — full 2K is the authoritative claim, computed LIVE in the wrap.** The wrap cell runs all four approaches over the 2K ESCI test queries with a progress bar (a few minutes on the cloned attendee machine; exact range verified at pilot) and computes **bootstrap 95% CIs**. The headline table leads with nDCG@10 and delta vs BM25; MRR@10, Recall@10, and Precision@10 are supporting checks from the same run. The CP2/CP3 in-room metrics come from the 10-query **demo set** (illustrative, n=10) — clearly labeled as such. There is no separate 200-query subset.
...
|---|---|---|
| 3 min | **Hook** | Live failure demo: "65 lg tv" query on generic dense returns the 55-inch variant at rank 1; the 65-inch (Exact) is buried at rank 6. One slide, one bad result, no math yet. |
| 10 min | **Concepts (one-liners)** | Search stack → dense embeddings → HNSW → sparse embeddings / BM25 / SPLADE → hybrid fusion → how this SPLADE model was trained. Each stays shallow; full depth is deferred to Q&A or the takeaway notebook. |
| 4 min | **Lab dataset + roadmap** | Explain ESCI, the workshop-specific 2K-query / ~38K-product subset, then preview the checkpoints and lift numbers. |
| 3 min | **Machine check-in** | Open the built-in IDE, open `notebooks/lab.ipynb`, run the Setup cell, and confirm it reaches the `Ready` summary. |

### Hands-on — 60 min

| Time | Checkpoint | What attendees do |
|---|---|---|
| 12 min | **CP1 — BM25 baseline (qualitative)** | Run the ~10 diversified demo queries against the `products` collection's `bm25` named vector. **No aggregate metric yet** — pure result inspection. Each result row shows its ESCI grade (E/S/C/I). The notebook includes short baseline-behavior labels for offline review; the instructor talks through the examples live. |
| 20 min | **CP2 — BM25, generic dense, and fine-tuned SPLADE (first numbers)** | Same queries against BM25, the `dense` named vector, and the `splade_finetuned` named vector. Side-by-side three-way view with ESCI grades on each row. Inspect one fine-tuned sparse vector — show its active terms (sparse is interpretable; dense is opaque). **First metric reveal:** nDCG@10, MRR@10, Recall@10, and Precision@10 across **BM25, generic dense, and fine-tuned SPLADE** on the **10-query demo set**, with plain product-search explanations — explicitly labeled as illustrative (n=10), not the headline claim. |
| 13 min | **CP3 — Hybrid fusion** | Use Qdrant's Query API to fuse generic dense + fine-tuned SPLADE with DBSF in a single `query_points` call — two `Prefetch` blocks + `FusionQuery(fusion=Fusion.DBSF)`. Re-run quality metrics **4-way** on the demo set (illustrative): BM25 / Generic dense / SPLADE / Hybrid DBSF. |
| 15 min | **Wrap, bad queries demo, Q&A** | Final headline table — **BM25 / Generic dense / SPLADE / Hybrid DBSF** · nDCG@10 with 95% CIs and delta vs BM25 from the full 2K eval. MRR@10 / Recall@10 / Precision@10 appear as supporting metrics from the same run. Then show the query-level findings table: where SPLADE helped most, and where hybrid helped or hurt. No latency or cost columns; the workshop stays focused on quality (latency is a Q&A topic, not a column). **Bad queries demo (~3 min):** five bad queries against fine-tuned SPLADE return product results regardless of intent — corpus-mismatch story, not a "model is broken" story. Mention query routing + cross-encoder rerank as the "next layer" in Q&A. Stretch "your turn" if time. Preview the "fine-tune your own" path with the takeaway notebook. |

## Workshop machine setup (target state at minute 0)

- Qdrant running and healthy, with one `products` collection populated by `scripts/setup_collections.py` from the ESCI test split (roughly 35K-40K unique products — every product appearing in any test-query qrels row). Every point carries three named vectors:
...
- Eval helper module imported and warm:
  - `ndcg_at_k`, `mrr_at_k`, `recall_at_k`, `precision_at_k` — quality metrics
  - `compare_results(query, models=[...])` — side-by-side result viewer with ESCI grade per row
- Retrieval helper module imported and warm:
  - `SpladeEncoder` — loads the custom SPLADE model for sparse query vectors
...
- Eval helper module (nDCG@10, MRR@k, Recall@k, Precision@k, side-by-side viewer with grades).
- Retrieval helper module (custom SPLADE encoder).
...
- **Confirm the generic-dense hook still lands on the indexed corpus.** For each demo query in `data/demo_queries.json`, run generic dense against the `products` collection and confirm the rank-of-first-Exact is ≥ 4. The failure demo is load-bearing — if MiniLM happens to surface the Exact within the top 3 on the actual Qdrant/HNSW retrieval, replace that query. "65 lg tv" → 55-inch is the highest-stakes verification. Same check for SPLADE recovering the Exact into top-3 on the focus demo queries.
- **Confirm the live lift is visible:** BM25 / generic dense → fine-tuned SPLADE should show a meaningful jump even at n=10 on the demo set. The headline lift is the 2K eval — but if the demo numbers don't move, revisit query selection.
...
- **Verify the full 2K eval shows a defensible win** (headline nDCG@10 plus bootstrap 95% CIs). The full 2K is the authoritative claim; the demo-set metrics are only for live feel.
...
- **Dense HNSW tuning.** Exact dense search matched default dense search on all 2,000 eval queries in the pilot check, so HNSW search parameters are not a quality lever for this corpus. Dense quality is model-limited here.
